...
const inputClass =
  "bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-2xl px-4 py-3 text-gray-800 dark:text-foreground";
const labelClass =
  "text-sm font-semibold text-gray-700 dark:text-muted-foreground mb-1.5";
...
export default function CreatePropertyScreen() {
  ...
  const showAlert = (
    ...
    setAlertConfig({ visible: true, title, message, type, onCloseCallback });
  };
  ...
  const handlePickImages = async () => {
    ...
    for (const asset of result.assets) {
      try {
        ...
        showAlert("Upload Failed", "One or more images failed to upload.");
      }
    }
  ...
  const handleDetectLocation = async () => {
    ...
    try {
      ...
      showAlert("Error", "Could not detect location. Enter manually.");
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.price.trim())
      return showAlert("Validation", "Price is required.");

    const priceNum = Number(form.price);
    if (isNaN(priceNum) || priceNum < MIN_PRICE)
      return showAlert("Validation", "Price must be greater than $0.");
    ...
    if (!form.address.trim())
      return showAlert("Validation", "Address is required.");
    if (!form.city.trim())
      return showAlert("Validation", "City is required.");
    if (form.images.length === 0)
      return showAlert("Validation", "Please upload at least one image.");
    ...
    if (error) {
      showAlert("Error", "Failed to create property. Please try again.");
      console.error(error);
      return;
    }

    setForm(INITIAL_FORM);
    showAlert("Success! 🎉", "Property listed successfully.", "success", () => {
  ...
  const Counter = ({
    ...
    <View className="flex-1">
      ...
      <View className="flex-row items-center bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
        <TouchableOpacity
        ...
        <Text className="flex-1 text-center text-gray-800 dark:text-foreground font-bold text-base">
          {value}
        </Text>
        <TouchableOpacity
    ...
    <TouchableOpacity
      ...
      className={`flex-row items-center justify-between p-4 rounded-2xl border ${
        value
          ...
          : "bg-white dark:bg-card border-gray-200 dark:border-gray-800"
      }`}
    >
      <View className="flex-1 mr-3">
        ...
        {description && (
          <Text className="text-xs text-gray-400 dark:text-muted-foreground mt-0.5">
            {description}
          </Text>
        )}
      </View>
    ...
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-background">
      ...
      >
        ...
        <View className="flex-row items-center px-5 pt-4 pb-3">
          <Text className="text-2xl font-bold text-gray-900 dark:text-foreground flex-1">
            Add Property
          </Text>
        </View>
        ...
        >
            ...
            <View className="flex-row flex-wrap gap-3">
              ...
              {form.localImages.length < 6 && (
                <TouchableOpacity
                  ...
                  className="w-24 h-24 rounded-2xl bg-white dark:bg-card border-2 border-dashed border-gray-300 dark:border-gray-800 items-center justify-center"
                >
                  {uploadingImages ? (
                    ...
                    <>
                      ...
                      />
                      <Text className="text-gray-400 dark:text-muted-foreground text-xs mt-1">
                        Add
                      </Text>
                    </>
                  )}
          ...
          {/* Price */}
          <View className={sectionClass}>
            <Text className={labelClass}>Price (USD)</Text>
            <TextInput
              className={inputClass}
          ...
          <View className={sectionClass}>
            <View className="flex-row items-center justify-between mb-1.5">
              ...
              <TouchableOpacity
                ...
                className="flex-row items-center gap-1 bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 rounded-full"
              >
                {detectingLocation ? (
                  <ActivityIndicator size="small" color="#2563EB" />
                ) : (
